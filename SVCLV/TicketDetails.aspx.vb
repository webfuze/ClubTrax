...
Public Class TicketDetails
	...
	Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
		...
		If Not IsPostBack Then
			...
			If Request.QueryString("ticketid") > 0 Then
				'edit mode
				Me.BindRepeater()

				strSQL = "SELECT * FROM tickets WHERE ticketid=" & Convert.ToString(Request.QueryString("ticketid"))
				...
				While dr.Read
					...
					strSQL = "SELECT   * FROM Locations  WHERE (locations.locationid NOT IN (SELECT locationid FROM ticketLocations WHERE (ticketid = '" & Request.QueryString("ticketid") & "'))) "    '  only get locations for the company they are logged in as*************************************************************
	...
	Protected Sub cmdSave_Click(sender As Object, e As EventArgs)
		...
		Else
			...
			strSQL = "UPDATE Tickets SET companyid=" & usercompanyid & ", customerid=" & usercustomerid & ",tickettypeid=" & TicketType.SelectedValue & ",statusid=" & StatusType.SelectedValue & ",priorityid=" & Priority.SelectedValue & ",subject='" & subject.Text & "',description='" & description.Text & "',datedue='" & txtDueDate.Text & "' WHERE ticketid=" & Request.QueryString("ticketid")
			...
			If Clarify.Text <> "" Then
				...
				strSQL = "INSERT INTO clarifications (ticketid,userid,text,datecreated) VALUES (" & Request.QueryString("ticketid") & "," & Session("gUserID") & ",'" & Clarify.Text & "','" & Now() & "')"
				SQLHelper.ExecuteScalar(SQLHelper.SQLConnection, CommandType.Text, strSQL)
			End If
